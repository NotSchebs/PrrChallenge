---
title: " Die Wahrscheinlichkeit die Schweizer Fussballmeisterschaft (Superleague) zu gewinnen"
author: "PRR Projekt von Alejandro Scheifele, Sébastien Bagdasarianz "
date: "`r Sys.Date()`"
output: 
  html_notebook:
    toc: true
...
<ul>
<li>Die Wahrscheinlichkeit, basierend auf dem Marktwert der einzelnen Spieler und der gesamten Mannschaft die Super League zu gewinnen.</li>
    
<li>Die Tabellenposition in Relation zu den erzielten und erhaltenen Toren.</li>

<li>Die Wahrscheinlichkeit, einen Platzierungsbereich zweimal in Folge zu erreichen.</li>
</ul>

# Zieldefinition

Mit dieser Analyse zeigen wir anhand der beiden Faktoren Marktwert und aktueller Position in der Meisterschaftstabelle (Ranking) die Wahrscheinlichkeit für den Gewinn der aktuellen Meisterschaft.

<ul>

<li>Anhand des Marktwerts</li>

<ul>

<li>Wahrscheinlichkeit einer Platzierung in den Top 3 mit einem Top-3 -Marktwert</li>

<li>Wahrscheinlichkeit einer Platzierung in den Top 5 mit einem Top-5 -Marktwert</li>

<li>Wahrscheinlichkeit einer Top 5 Platzierung mit den 5 tiefsten Marktwerten</li>
...
<li>Wahrscheinlichkeit eines Titelgewinns in Folge</li>

<li>Wahrscheinlichkeit, zweimal nacheinander in die Top 3 zu kommen</li>

<li>Relation von Torverhältnis und Ranking</li>

<li>Relation der geschossenen Tore zum Ranking</li>

<li>Relation der erhaltenen Gegentore zum Ranking</li>
...
Wir haben auf der Website von Transfermarkt die Daten aller Saisons von 2004 bis
2024 in einen Dataframe zusammengefasst und diesen importiert.<br>

-   <code>#renv::init()</code>: Initialisiert ein neues renv-Projekt, das eine isolierte und reproduzierbare R-Umgebung erstellt.

-   <code>renv::snapshot()</code>: Speichert den aktuellen Zustand der installierten Bibliotheken in einer renv.lock-Datei, um die Umgebung später reproduzieren zu können.

-   <code>renv::restore()</code>: Stellt die Umgebung basierend auf der renv.lock-Datei wieder her, indem fehlende Pakete installiert werden.

-   **Pakete laden**: Es werden zahlreiche Pakete wie dplyr, ggplot2, und tidymodels geladen, die für Datenmanipulation, Visualisierung, maschinelles Lernen und mehr genutzt werden.

-   **Daten laden**: Die CSV-Datei wird mit read.csv eingelesen und in das Objekt data gespeichert. Dieses wird anschliessend in DF kopiert.

```{r}

#renv::init()
#renv::snapshot() #Renv snapshot zur integration der Library
renv::restore()   #Renv restore zur kontrolle ob die entsprechenden Packete installiert sind, falls nicht werden sie Installiert
...
<p>Die Werte wurden anschliessend in numerische Variablen umgewandelt, um
...
<br>Es werden neue Spalten erstellt mit dem durchschnittlichen Marktwert pro Saison und dem Gewinnverhältnis von jeder Mannschaft. Der durchschnittliche Marktwert wird für zukünftige Berechnungen benötigt und hier als Punktdiagramm dargestellt.
...
<br>**Vorgehen** <br> Zunächst werden die Top-3-Teams nach Marktwert pro Saison identifiziert (also die drei Teams mit dem höchsten Marktwert in jeder Saison). Anschliessend werden die Top-3-Mannschaften der jeweiligen Saison ermittelt und mit den Top-3-Marktwert-Teams verglichen.
...
Auf Grundlage eines Modells, das auf logistischer Regression und Monte-Carlo-Simulationen basiert, werden schliesslich Vorhersagen für zukünftige Saisons getroffen.



## Die Berechnung

<br> Um die Wahrscheinlichkeit zu berechnen, dass ein Team mit einem der drei höchsten Marktwerten in den Top 3 der Liga landet, wird schrittweise vorgegangen

Zunächst wird das Verhältnis der Teams mit den höchsten Marktwerten, die tatsächlich unter den Top 3 platziert waren, visualisiert. Anschliessend wird die Wahrscheinlichkeit berechnet: <br><ul> <li>**Gesamtzahl der Teams mit höchstem
...
<strong>unterdurchschnittlichen</strong> Marktwert. - Werte grösser als
...
bedeutet es  dass die Top-Teams einen überdurchschnittlich hohen
...
es, dass die Teams ausserhalb der Top `a` einen unterdurchschnittlichen
...
`1` und einigen hohen Werten)bedeutet dies, dass nur wenige Teams extrem
hohe Marktwerte haben. Wenn die Verteilung eher gleichmässig ist,
bedeutet es, dass es keine grossen Unterschiede zwischen den Teams
gibt.</p>
...
-   Der Koeffizient für GD (Tordifferenz) zeigt, dass mit jedem Anstieg
    der Tordifferenz um 1 die Wahrscheinlichkeit für ein Team, in den
...
<li><strong>Binäre Variable (<code>Top</code>):</strong> Anschliessend
...
verschiedenen Marktwerten, Gewinnverhältnissen und Tordifferenzen
ergeben können.</p>
...
Die Auswertungen zeigen eine ähnliche Verteilung wie bei den Top 3, allerdings sind die Marktwerte geringer. Dies verdeutlicht, dass die Top 5 im Vergleich zum Marktwert weniger konsistent sind und es grössere Unterschiede zwischen dem höchsten und dem niedrigsten Marktwert gibt.
...
<p>Schliesslich wird überprüft, ob der Club in der nächsten Saison
...
<p>Abschliessend wird ein Diagramm erstellt, das zeigt, ob ein Club
...
-   Die Daten werden gefiltert, sodass nur Meisterteams (Rank == 1) berücksichtigt werden. Anschliessend wird mit der lead()-Funktion überprüft, ob der Meistertitel im nächsten Jahr verteidigt wurde. Ergebnisse werden in der Spalte Repeated gespeichert.
...
plot <- ggplot(data, aes(x = Marktwert_in_Mio_Euro, y = Pts, color = Club)) +
  ...
  theme(
    ...
    plot.margin = unit(c(1, 1, 2, 1), "cm"), # Platz am unteren Rand vergrössern
...
-   Der Mean Absolute Error (MAE) ist ein Mass für die durchschnittliche Abweichung der Vorhersagen vom tatsächlichen Wert. Es wird berechnet als der Mittelwert der absoluten Differenzen zwischen den vorhergesagten Punkten (predictions) und den tatsächlichen Punkten im Testdatensatz (test_data$Pts).
...
-   Konservativ: Der Marktwert bleibt weitgehend unverändert, ausser für den saisonalen Faktor.
...
-   Die simulierten Ergebnisse für jedes Jahr werden in einer Liste gespeichert und anschliessend zu einem einzelnen Datensatz zusammengeführt.
...
-   Die Ergebnisse für jedes Szenario werden in den Variablen future_optimistic, future_conservative und future_pessimistic gespeichert und anschliessend angezeigt.
...
## Zusammenfassung und Interpretation der Szenarien

Die Simulation der zukünftigen Tabellenplatzierungen zeigt deutliche Unterschiede zwischen den drei betrachteten Szenarien (optimistisch, konservativ, pessimistisch). Diese Szenarien basieren auf Veränderungen der Marktwerte und ihrer Auswirkungen auf die Leistung der Teams.

-   Optimistisches Szenario: Ein Anstieg der Marktwerte führt zu einer deutlichen Leistungssteigerung bei den meisten Teams. Die Top-Clubs wie BSC Young Boys und FC Basel 1893 dominieren weiterhin, jedoch profitieren auch kleinere Teams von verbesserten Marktwerten. Insgesamt zeigt dieses Szenario eine stärkere Wettbewerbsfähigkeit und höhere Punktzahlen in der Liga.
-   Konservatives Szenario: In diesem Szenario bleiben die Marktwerte weitgehend unverändert. Die Tabelle zeigt eine stabile Leistung der Teams, mit nur geringfügigen Verschiebungen in den Platzierungen. Dieses Szenario repräsentiert den Status quo und ist nützlich, um die aktuellen Leistungsdynamiken in der Liga zu bewerten.
-   Pessimistisches Szenario: Ein Rückgang der Marktwerte führt zu spürbaren Leistungseinbrüchen, insbesondere bei Teams, die auf hohe Investitionen angewiesen sind. Top-Clubs bleiben meist stark, jedoch kämpfen mittelmässige und kleinere Vereine mit deutlich schlechteren Ergebnissen. Das Szenario verdeutlicht, wie empfindlich Teams auf finanzielle Veränderungen reagieren.

## Interpretation
Die Simulationen unterstreichen die Bedeutung des Marktwerts als Schlüsselfaktor für den sportlichen Erfolg. Während ein Anstieg der Marktwerte zu einer insgesamt höheren Wettbewerbsfähigkeit führt, zeigt ein Rückgang deutliche Schwächen im System auf. Die konservative Prognose dient als realistischer Referenzpunkt, um die möglichen Extremszenarien einzuordnen.

Diese Ergebnisse können Vereinen und Entscheidungsträgern helfen, Strategien zur Optimierung von Marktwerten und sportlicher Leistung zu entwickeln. Sie verdeutlichen zudem, wie wirtschaftliche Faktoren die Dynamiken einer Liga beeinflussen und dass eine gezielte Förderung insbesondere kleinerer Vereine langfristig die Stabilität und Attraktivität der Liga sichern kann.

Unsere Software simuliert mögliche Platzierungen für zukünftige Saisons, stellt jedoch keine Garantie dar. In der aktuellen Simulation wird YB Meister, obwohl das Team derzeit(stand 05.01.2025) auf Platz 9 der Superleague steht. Auffällig ist, dass Mannschaften wie FC Zürich und FC Basel weiterhin in der Spitze mitspielen, während Überraschungen wie FC Lugano und Servette FC, die in der Simulation nie Meister wurden, ebenfalls für Spannung sorgen.

# Quellen

-   https://www.transfermarkt.ch    (Zugriff am 11. November 2024)
-   https://studyflix.de    (Zugriff am 17. November 2024)
-   https://spaces.technik.fhnw.ch/storage/uploads/mml/2019/09/marcelsteinerskriptwahrscheinlichkeitstheorieundstatistik.pdf    (Zugriff am 11. November 2024)
-   https://rstudio.github.io/renv/articles/renv.html   (Zugriff am 22. November 2024)
-   https://rcourses.pages.fhnw.ch/programmieren-in-r/    (Zugriff am 16. November 2024)
